...
export default function CreditTracker({
  ...
  const allDebtors = useMemo(() => {
    ...
    // Calculate remaining balance for each debtor
  ...
  // ── Total outstanding (only unsettled) ───────────────────────────────────
  ...
  return (
    <div style={{ paddingBottom: '100px' }}>
      ...
      {/* Summary banner */}
      {totalOutstanding > 0 && (
        <div style={{ background:'#FEF3C7', borderRadius:'12px', padding:'12px 14px', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'12px', border:'1px solid #FDE68A' }}>
          <div>
            <p style={{ margin:'0 0 2px', fontSize:'11px', fontWeight:700, color:'#92400E', textTransform:'uppercase', letterSpacing:'0.05em' }}>Total Outstanding</p>
          ...
          <div style={{ textAlign:'right' }}>
            <p style={{ margin:'0 0 2px', fontSize:'11px', color:'#92400E' }}>
              {allDebtors.filter(d => d.remaining > 0).length} debtor{allDebtors.filter(d => d.remaining > 0).length !== 1 ? 's' : ''}
            </p>
            <p style={{ margin:0, fontSize:'11px', color:'#B45309' }}>{overdueCount} overdue &gt;14 days</p>
          </div>
        </div>
      )}

      {allDebtors.filter(d => d.remaining === 0).length === allDebtors.length && allDebtors.length > 0 && (
        <div style={{ textAlign:'center', padding:'24px 20px 8px', color:'#9CA3AF' }}>
          <div style={{ fontSize:'32px', marginBottom:'6px' }}>✅</div>
          <p style={{ fontWeight:700, color:'#111827', fontSize:'15px', margin:'0 0 4px' }}>All accounts settled</p>
          <p style={{ fontSize:'12px', margin:0 }}>No outstanding balances</p>
        </div>
      )}
      ...
      <div style={{ position:'relative', marginBottom:'10px' }}>
        ...
        <input type="text" placeholder="Search debtors..." value={search}
          ...
          style={{ width:'100%', padding:'10px 12px 10px 34px', borderRadius:'10px', border:'1.5px solid #E5E7EB', fontSize:'14px', color:'#111827', background:'white', outline:'none', boxSizing:'border-box' }} />
        {search && (
      ...
      <div style={{ display:'flex', gap:'6px', flexWrap:'wrap', marginBottom:'14px' }}>
        {[
          ...
          <button key={f.key} onClick={() => setActiveFilter(f.key)} style={{
            ...
            color: activeFilter === f.key ? '#4F6EF7' : '#6B7280', cursor:'pointer'
          }}>{f.label}</button>
        ))}
      </div>

      {filteredDebtors.length === 0 && allDebtors.length > 0 && (
      ...
      {filteredDebtors.map(debtor => {
        ...
        const isSettled     = debtor.remaining === 0
        ...
        return (
          <div key={debtor.name} style={{ background:'white', borderRadius:'14px', marginBottom:'10px', boxShadow:'0 1px 8px rgba(0,0,0,0.07)', overflow:'hidden', border: isSettled ? '1.5px solid #A7F3D0' : '1px solid transparent' }}>
            ...
            <div style={{ padding:'14px 14px 10px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'8px' }}>
                <div>
                  <button onClick={() => setExpandedCustomer(isExpanded ? null : debtor.name)}
                    style={{ background:'none', border:'none', padding:0, cursor:'pointer', fontWeight:700, fontSize:'15px', color:'#111827', textDecoration:'underline', textDecorationStyle:'dotted', textDecorationColor:'#9CA3AF', textUnderlineOffset:'3px' }}>
                    {debtor.name}
                  </button>
                ...
                <div style={{ textAlign:'right' }}>
                  {isSettled ? (
                    <span style={{ fontSize:'13px', fontWeight:700, color:'#059669', background:'#ECFDF5', padding:'4px 10px', borderRadius:'20px', border:'1px solid #A7F3D0' }}>
                      ✅ Settled
                    </span>
                  ) : (
                    <>
                      <p style={{ margin:0, fontSize:'16px', fontWeight:800, color:'#EF4444' }}>{fmt(debtor.remaining)}</p>
                      {debtor.totalPaid > 0 && (
                        <p style={{ margin:'2px 0 0', fontSize:'11px', color:'#10B981' }}>{fmt(debtor.totalPaid)} paid</p>
                      )}
                    </>
                  )}
                </div>
              ...
              {pct > 0 && (
                <div style={{ height:'4px', borderRadius:'2px', background:'#F3F4F6', marginBottom:'10px', overflow:'hidden' }}>
                  <div style={{ height:'100%', width:`${pct}%`, background: isSettled ? '#10B981' : '#4F6EF7', borderRadius:'2px', transition:'width 0.4s ease' }} />
                </div>
              )}
              ...
              <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                {!isSettled && (
                  <>
                    <button onClick={() => setPartialCustomer(isPartialOpen ? null : debtor.name)} style={actionBtn('#4F6EF7')}>
                      💰 Part Pay
                    </button>
                    <button onClick={() => handleMarkFullyPaid(debtor)} style={actionBtn('#059669')}>
                      ✅ Mark Fully Paid
                    </button>
                ...
                {debtor.sales.some(s => parseInt(s.crates_loaned||0) - parseInt(s.crates_returned||0) > 0) && (
                  <button onClick={() => setExpandedCustomer(isExpanded ? null : debtor.name)} style={actionBtn('#D97706')}>
                    📦 Return Crates
                  </button>
                )}
              </div>
            ...
            {isPartialOpen && (
              <div style={{ padding:'12px 14px', borderTop:'1px solid #F3F4F6', background:'#F9FAFB' }}>
                <p style={{ margin:'0 0 10px', fontSize:'12px', fontWeight:700, color:'#374151' }}>Record partial payment</p>
                ...
                <div style={{ padding:'12px 14px', background:'#F9FAFB', borderBottom:'1px solid #F3F4F6' }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'8px' }}>
                    <div style={{ textAlign:'center' }}>
                      <p style={{ margin:'0 0 2px', fontSize:'10px', fontWeight:600, color:'#9CA3AF', textTransform:'uppercase' }}>Original Sale</p>
                      <p style={{ margin:0, fontSize:'14px', fontWeight:700, color:'#111827' }}>{fmt(debtor.originalTotal)}</p>
                    </div>
                    <div style={{ textAlign:'center', borderLeft:'1px solid #E5E7EB', borderRight:'1px solid #E5E7EB' }}>
                      <p style={{ margin:'0 0 2px', fontSize:'10px', fontWeight:600, color:'#9CA3AF', textTransform:'uppercase' }}>Total Paid</p>
                      <p style={{ margin:0, fontSize:'14px', fontWeight:700, color:'#10B981' }}>{fmt(debtor.totalPaid)}</p>
                    </div>
                    <div style={{ textAlign:'center' }}>
                      <p style={{ margin:'0 0 2px', fontSize:'10px', fontWeight:600, color:'#9CA3AF', textTransform:'uppercase' }}>Outstanding</p>
                      <p style={{ margin:0, fontSize:'14px', fontWeight:700, color: debtor.remaining > 0 ? '#EF4444' : '#059669' }}>
                        {debtor.remaining > 0 ? fmt(debtor.remaining) : '₦0 ✓'}
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                        <div>
                          <div style={{ display:'flex', alignItems:'center', gap:'6px', flexWrap:'wrap' }}>
                            <span style={{ fontSize:'10px', fontWeight:700, padding:'2px 7px', borderRadius:'20px', background:aging.bg, color:aging.color }}>
                              {aging.label}
                            </span>
                          ...
                          {cratesStillOut > 0 && (
                            <div style={{ marginTop:'8px', display:'flex', alignItems:'center', gap:'6px', flexWrap:'wrap' }}>
                              ...
                              <input type="number" inputMode="numeric"
                                placeholder={`max ${cratesStillOut}`}
                                value={returningSaleId===sale.id ? returnQty : ''}
                                onChange={e => { setReturningSaleId(sale.id); setReturnQty(e.target.value) }}
                                style={{ width:90, padding:'4px 8px', borderRadius:'7px', border:'1.5px solid #D97706', fontSize:'12px', outline:'none', background:'#FFFBEB' }}
                              />
                              <button onClick={() => handleCrateReturn(sale)}
                                disabled={returning||returningSaleId!==sale.id||!returnQty}
                                style={{ padding:'4px 10px', borderRadius:'7px', background:returning?'#F3F4F6':'#D97706', color:returning?'#9CA3AF':'white', border:'none', fontSize:'11px', fontWeight:700, cursor:'pointer' }}>
      ...
      {cratesOutCustomers.length > 0 && (
        <div style={{ marginTop:'20px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'12px' }}>
            ...
            <span style={{ fontSize:'11px', background:'#FFFBEB', color:'#D97706', border:'1px solid #FDE68A', borderRadius:'20px', padding:'2px 8px', fontWeight:600 }}>
              {cratesOutCustomers.reduce((s,c) => s+c.totalOut, 0)} total
            </span>
          </div>
          {cratesOutCustomers.map(customer => (
            <div key={customer.name} style={{ background:'white', borderRadius:'14px', marginBottom:'10px', boxShadow:'0 1px 8px rgba(0,0,0,0.07)', overflow:'hidden' }}>
                  ...
                  <div key={sale.id} style={{ marginBottom:'10px' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                      ...
                      <input type="number" inputMode="numeric"
                        ...
                        value={returningSaleId===sale.id?returnQty:''}
                        onChange={e => { setReturningSaleId(sale.id); setReturnQty(e.target.value) }}
                        style={{ width:90, padding:'5px 8px', borderRadius:'7px', border:'1.5px solid #D97706', fontSize:'12px', outline:'none', background:'#FFFBEB' }}
                      />
                      <button onClick={() => handleCrateReturn(sale)}
                        disabled={returning||returningSaleId!==sale.id||!returnQty}
                        style={{ padding:'5px 12px', borderRadius:'7px', background:(returning&&returningSaleId===sale.id)?'#F3F4F6':'#D97706', color:(returning&&returningSaleId===sale.id)?'#9CA3AF':'white', border:'none', fontSize:'12px', fontWeight:700, cursor:'pointer' }}>
                        {returning&&returningSaleId===sale.id?'...':'Confirm'}
                      </button>
                    </div>
                  </div>
...
const actionBtn = (color) => ({
  ...
  background:`${color}12`, color, border:`1px solid ${color}30`, cursor:'pointer'
})
const lblStyle = {
