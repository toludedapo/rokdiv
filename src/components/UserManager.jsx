...
export default function UserManager({ adminEmail }) {
  ...
  async function fetchUsers() {
    setLoading(true)
    try {
      // Uses Supabase Admin API via the anon key — only works if RLS allows
      // We call the auth admin endpoint via a Supabase Edge Function alternative:
      // Instead, we list users from our own profiles or auth.users via service role.
      // Since we only have anon key in the client, we use a workaround:
      // Store invited/active users in a public 'app_users' table we can read.
  ...
  async function handleInvite() {
    ...
    try {
      ...
      // If admin API not available from client (expected), use signInWithOtp as invite
      if (error && error.message.includes('not allowed')) {
        // Fallback: use magic link OTP — user gets an email to set up their account
  ...
  return (
    <div className="mx-4" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ ...card, padding: '16px 18px' }}>
        ...
        {error && (
          <div style={{ marginTop: 10, background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: '9px 13px', fontSize: 12, color: '#DC2626' }}>
            {error}
          </div>
        )}
        {success && (
          <div style={{ marginTop: 10, background: '#ECFDF5', border: '1px solid #A7F3D0', borderRadius: 10, padding: '9px 13px', fontSize: 12, color: '#059669', fontWeight: 600 }}>
            ✓ {success}
          </div>
        )}

        <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: 10 }}>
          The user will receive a magic link by email to set their password and log in.
        </p>
      </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {u.email}
                    {u.email === adminEmail && (
                      <span style={{ marginLeft: 6, fontSize: 10, background: '#EEF1FF', color: '#4F6EF7', borderRadius: 99, padding: '1px 7px', fontWeight: 700 }}>
                        Admin
                      </span>
                    )}
                  </p>
